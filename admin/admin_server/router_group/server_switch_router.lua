...
container_client:register("server_info_m")
...
return function(group)
    ...
    group:get('/infos',function(c)
        local infos = container_client:instance('server_info_m'):mod_call('get_all_server_info')
    ...
    group:get('/white_info', function(c)
        local map = container_client:instance('server_info_m'):mod_call('get_white_map')
    ...
    group:post('/add_white', function(c)
        ...
        local ret, errno, errmsg = container_client:instance('server_info_m'):mod_call('add_white', player_id)
        if not ret then
    ...
    group:post('/del_white', function(c)
        local body = c.req.body
        local player_id = body.player_id or 0
        local ret, errno, errmsg = container_client:instance('server_info_m'):mod_call('del_white', player_id)
        if not ret then
