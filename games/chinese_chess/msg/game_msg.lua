...
function M:game_state_res(player_id,game_state_res)
    if player_id then
        ...
        self.interface_mgr:broadcast('.chinese_chess_game.gameStateRes',game_state_res)
    end
end

function M:next_doing(next_doing)
    self.interface_mgr:broadcast('.chinese_chess_game.nextDoing',next_doing)
end

function M:move_res(move_res)
    self.interface_mgr:broadcast('.chinese_chess_game.moveRes',move_res)
end
