...
local M = {}
local mt = {__index = M, __gc = function()
...
function M:on_connect(player_id, table_id, token, send_msg)
    ...
    send_msg('.chinese_chess_login.LoginReq', login_req)
end
...
function M:on_handle(packname, packbody)
    local HANDLE_FUNC = {}
     --游戏服登录成功
     HANDLE_FUNC['.chinese_chess_login.LoginRes'] = function(body)
        --发送心跳包
        if self.m_heart_timer then
            self.m_heart_timer:cancel()
        end
        ...
        self.m_heart_timer = timer:new(timer.second * 5, 0, function()
            ...
            self.m_send_msg('.chinese_chess_hall.HeartReq', heart_req)
        end)
        ...
        if body.isreconnect == 1 then   --如果是重连
            ...
            --请求进入桌子
            self.m_send_msg('.chinese_chess_hall.JoinReq', {
    ...
    --进入桌子成功
    HANDLE_FUNC['.chinese_chess_hall.JoinRes'] = function()
        --请求游戏状态
        self.m_send_msg('.chinese_chess_game.gameStateReq', {
            player_id = self.m_player_id
        })
