...
return {
	...
	share_config_m = {
		...
		default_arg = {     --默认配置
			...
			--cluster_server用的配置
			frpc_server = frpc_server_cfg.world.loginserver_1,
	...
	logrotate_m = {
        ...
        launch_num = 1,
        default_arg = {
            file_path = server_cfg.world.loginserver_1.logpath,          --文件路径
            filename = 'server.log',   --文件名
            limit_size = 0,            --最小分割大小
            max_age = 7,               --最大保留天数
            max_backups = 7,           --最大保留文件数
            sys_cmd = [[
                /usr/bin/pkill -HUP -f skynet.make/loginserver_config.lua.load_mods_1.lua\n
            ]],              --系统命令
        }
    },
